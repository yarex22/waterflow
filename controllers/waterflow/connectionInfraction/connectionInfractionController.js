...
const ConnectionInfraction = require('../../../models/waterflow/connectionInfraction/ConnectionInfractionModel');
...
// @route   POST /api/connection-infractions
exports.createConnectionInfraction = asyncHandler(async (req, res, next) => {
  ...
  try {
    // Criar a infração
    const infraction = await ConnectionInfraction.create({
      connection,
      infractionType,
      images: imagePaths,
...
// @route   GET /api/connection-infractions
exports.getAllConnectionInfractions = asyncHandler(async (req, res, next) => {
  const {
    pageSize = 10,
    pageNumber = 1,
    searchTerm,
  ...
  try {
    ...
    const totalCount = await ConnectionInfraction.countDocuments(query);

    // Buscar infrações com paginação
    const infractions = await ConnectionInfraction.find(query)
      .populate({
        path: 'connection',
        populate: {
          path: 'customer',
...
// @route   GET /api/connection-infractions/:id
exports.getConnectionInfractionById = asyncHandler(async (req, res, next) => {
  const { id } = req.params;
  ...
  const infraction = await ConnectionInfraction.findOne(query)
    .populate({
      path: 'connection',
      populate: {
        path: 'customer',
...
// @route   PUT /api/connection-infractions/:id
exports.updateConnectionInfraction = asyncHandler(async (req, res, next) => {
  const { id } = req.params;
  ...
  try {
    ...
    const existingInfraction = await ConnectionInfraction.findOne(query);
    ...
    const infraction = await ConnectionInfraction.findOneAndUpdate(
      query,
      updateData,
      { 
        new: true, 
...
// @route   PATCH /api/connection-infractions/:id/status
...
// @route   GET /api/connection-infractions/company/:companyId
...
// @route   GET /api/connection-infractions/connection/:connectionId
...
// @route   DELETE /api/connection-infractions/:id
exports.deleteConnectionInfraction = asyncHandler(async (req, res, next) => {
  const { id } = req.params;
  ...
  const infraction = await ConnectionInfraction.findOneAndDelete(query);
